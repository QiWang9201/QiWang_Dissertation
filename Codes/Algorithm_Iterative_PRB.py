...
def AIPRB(filename):
    ...
    df = pd.read_csv('./Epias/veri_1.in', header=None, names = column_names)
                ...
                if len(set(keys) & set(BK)) == 0: # no such c exists
                    ...
                    for z in Z:
                        if Delta[b][z-1] > 0:# z_th hour is impacted by block b
                            y[z] = y[z] - Bbid[b][2]
                            ans = CP.get_inisol(x[z], y[z])
                            #ans = CP.get_cpsol(x[z], y[z], -Bbid[b][2])
                        else:
                            ans = curr_p[z-1]
                        curr_p[z-1] = ans
    ...
    flag9 = 1
    # calculate the impact value for bids in BK and FK
    while flag9 == 1:
        impact_value_9 = {}
        for b in BK:
            avg_cp = sum(np.array(Delta[b])*curr_p)/Bbid[b][1]
        ...
        # Step 9.1 for all bids in BK and FK, sort the impact value in increasing order
        sorted_dict = sorted(impact_value_9.items(), key=lambda item: item[1])
        ...
        for b in impact_value_9.keys():
            if impact_value_9[b] < 0:
                if b in Order_id_B:
                    keys9 = [key for key, value in Kb.items() if value == b]
                    if len(keys9) == 0 or set(keys9)&set(BR) == set(keys9):
                        BK.remove(b)
                        BR.append(b)
                        add_in_9 = b
                        for z in Z:
