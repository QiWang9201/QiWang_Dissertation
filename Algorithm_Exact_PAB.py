...
def AEPAB(filename):
    ...
    # block accpetance rule(PAB)
    m_exact.addConstrs(- Bbid[b][1] * Bbid[b][3] + gp.quicksum(Delta[b][z-1] * f[z] for z in Z) <= Gam * y[b] for b in no_child_S);
    m_exact.addConstrs( Bbid[b][1] * Bbid[b][3] - gp.quicksum(Delta[b][z-1] * f[z] for z in Z) <= Gam * y[b] for b in no_child_T);
    ...
    # limit the acceptance of a flexible bid to a maximum of one period
    m_exact.addConstrs(gp.quicksum(v[b,z] for z in Z) <= 1 for b in Order_id_F);
    m_exact.addConstrs(f[z] - Fbid[b][1] <= Gam * gp.quicksum(v[b,i] for i in Z) for b in ES for z in Z);
    m_exact.addConstrs(-f[z] + Fbid[b][1] <= Gam * gp.quicksum(v[b,i] for i in Z) for b in ET for z in Z);
    ...
    m_exact.write(filename[-5:-3]+'.sol') 
    # for z in Z:
